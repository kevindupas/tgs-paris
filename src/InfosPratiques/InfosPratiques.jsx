...
export default function InfosPratiques() {
  ...
  if (infos) {
    return (
      <>
        <CategoryImage
          categoryId={INFOS_PRATIQUES_CATEGORY}
          ...
          <div className="max-w-[1200px] w-[calc(100%-30px)] mx-auto">
            <div className="flex flex-wrap mt-[-20px] ml-[-20px]">
              {sortedData.map((value) => (
                <article
                  key={value.id}
                  className="pt-[20px] pl-[20px] w-full lg:w-1/3 lg:basis-auto text-black"
                >
                  <div className="py-[15px] px-[30px] w-auto h-80 mx-auto my-0 bg-zinc-200">
                    <div className="flex items-center flex-col">
                      ...
                      />
                      <h1 className="text-5xl mt-4 text-center font-tungstenb_semibold tracking-[0.566667px] leading-[.95] uppercase">
                        {value.name}
                      </h1>
                    </div>
                    ...
                    <div className="flex items-center justify-center">
                      <Link
                        key={value.id}
                        to={`/info/${value.id}`}
                        className="mt-4 text-sm flex items-end font-bold uppercase hover:text-[#E97665] hover:scale-125"
                      >
                        Lire la suite
                      </Link>
                    </div>
